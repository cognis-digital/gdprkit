# Comprehensive installer for cognis-digital/gdprkit (Windows PowerShell).
# Tries: pipx -> uv -> pip (git+https) -> from source.
# gdprkit is source-available and not on PyPI; all paths install from GitHub.
$ErrorActionPreference = "Stop"
$Repo = "gdprkit"
$Url  = "git+https://github.com/cognis-digital/gdprkit.git"
$Git  = "https://github.com/cognis-digital/gdprkit.git"
...
if (Have pipx) {
  ...
  pipx install $Url; if ($LASTEXITCODE -eq 0) { Say "Done. Run: gdprkit"; exit 0 }
}
if (Have uv) {
  Say "Installing with uv..."
  uv tool install $Url; if ($LASTEXITCODE -eq 0) { Say "Done. Run: gdprkit"; exit 0 }
}
if (Have pip) {
  Say "Installing with pip (user site)..."
  pip install --user $Url; if ($LASTEXITCODE -eq 0) { Say "Done. Run: gdprkit"; exit 0 }
}
