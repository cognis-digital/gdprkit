"""Native cognis-connect emit for gdprkit — forward findings to any platform.

Maps gdprkit's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    gdprkit ... --format json | gdprkit-emit --to stix
    gdprkit-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    ...
    try:
        ...
        elif 'ipv4' in rec or 'domain' in rec or 'url' in rec or 'sha256' in rec or 'imo' in rec or 'mmsi' in rec or 'lat' in rec or 'lon' in rec:
            out['severity'] = 'high'
        out['type'] = 'finding'
        out['description'] = rec.get('title', '')
